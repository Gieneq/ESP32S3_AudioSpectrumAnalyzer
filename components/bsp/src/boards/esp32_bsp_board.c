...
esp_err_t bsp_btn_init(void)
{
    ...
    ESP_LOGD(TAG, "Btn num: %d/%d", BSP_BUTTON_NUM, BSP_BUTTON_NUM);
    esp_err_t ret = bsp_iot_button_create(g_btn_handle, &btn_num, BSP_BUTTON_NUM);
    return ret; 
}
...
#ifdef CONFIG_BSP_BOARD_ESP32_S3_BOX_Lite
    ESP_LOGI(TAG, "Selected board lite.");
#else
    ESP_LOGW(TAG, "Selected board other! BAD.");
#endif
